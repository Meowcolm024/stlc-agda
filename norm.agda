...
⊢𝒩 : ∀ {Γ : Context n} {σ : Fin n → Term 0} {M A}
  ...
  → 𝒩 A ⟦ subst σ M ⟧
⊢𝒩 (⊢var x) σ⊨Γ = σ⊨Γ x
⊢𝒩 {σ = σ} {M = ƛ M} {A = A ⇒ B} (⊢abs ⊢M) σ⊨Γ = ⊢subst (⊢abs ⊢M) σ⊨Γ , halts (subst σ (ƛ M) ∎) V-abs , k
  where
    k : ∀ {N} → 𝒩 A ⟦ N ⟧ → 𝒩 B ⟦ (ƛ subst (exts σ) M) · N ⟧
    k {N} nn with 𝒩-halts nn
    ... | halts {N = N'} N—→*N' VN' = ⊢—→*𝒩' (⊢app (⊢subst (⊢abs ⊢M) σ⊨Γ) (𝒩→⊢ nn)) st (⊢𝒩 {σ = N' • σ} ⊢M (λ { Z → ⊢—→*𝒩 N—→*N' nn ; (S x) → σ⊨Γ x }))
      where
...
... | inj₁ refl = ⊢—→*𝒩' (⊢if ⊢σL (⊢subst ⊢M σ⊨Γ) (⊢subst ⊢N σ⊨Γ)) (—→*-trans (if-cong L—→*L') (step—→ (if true (subst σ M) (subst σ N)) (subst σ M ∎) β-if₁)) (⊢𝒩 ⊢M σ⊨Γ)
... | inj₂ refl = ⊢—→*𝒩' (⊢if ⊢σL (⊢subst ⊢M σ⊨Γ) (⊢subst ⊢N σ⊨Γ)) (—→*-trans (if-cong L—→*L') (step—→ (if false (subst σ M) (subst σ N)) (subst σ N ∎) β-if₂)) (⊢𝒩 ⊢N σ⊨Γ)
